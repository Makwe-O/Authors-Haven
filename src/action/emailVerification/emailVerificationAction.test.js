...
describe('Forgot Password Actions', () => {
  ...
  test('Dispatches the correct action and payload when a success response is return', () => {
    ...
    mock
      .onPost('https://adv-backend.herokuapp.com/api/v1/auth/resend-verification-link', {
  ...
  test('should make api call for forgot password', () => {
    ...
    const mock = new MockAdapter(Axios);
    const history = () => {};
    mock.onPost(`https://adv-backend.herokuapp.com/api/v1/auth/verify/${token}`).reply(500, {
      data: {
        status: 'failure'
      }
    });
